import { usePlayers, useBattingStats, usePitchingStats, useCoaches } from "@/hooks/use-supabase";
...
import { coachingStaff } from "@/content/coaches";

function PlayerCard({ player }: { player: Player }) {
  const isPitcher = player.position?.toLowerCase().includes("p") && !player.position?.toLowerCase().includes("dp");

  return (
    <Link href={`/player/${player.id}`} onClick={() => track('Player Click', { player: `${player.first_name} ${player.last_name}`, number: player.number })} data-testid={`player-card-${player.id}`}>
